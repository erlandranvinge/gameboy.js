...
Debugger.prototype.trace = function(count) {
    count = count || 5;
    for (var i = Math.max(0, this.history.length - count); i < this.history.length; i++) {
        this.deasm(this.history[i]);
    }
};

Debugger.prototype.record = function() {
    if (this.history.length > 0 && this.history[this.history.length - 1] === this.cpu.pc)
        throw 'Debugger: Inifite loop detected.';

    this.history.push(this.cpu.pc);
};
